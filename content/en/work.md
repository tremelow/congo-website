---
title: "Publications"
draft: false
description: "List of research publications of L. Trémant."
showDate: false
showAuthor: false
showReadingTime: false
showEdit: false
showPagination: false
---

{{< lead >}}
Preprint
{{< /lead >}}

- Philippe Chartier, Mohammed Lemou, Florian Méhats, LT, "Averaging in a Nutshell" (2024). To be submitted.
  {{< badge href="publications/avg-nutshell.pdf" >}}PDF{{< /badge >}}

{{< lead >}}
Journals
{{< /lead >}}

- Clémentine Courtès, Emmanuel Franck, Michael Kraus, Laurent Navoret, LT, "Neural non-canonical Hamiltonian dynamics for long-time simulations" (2025). Preprint.
  {{< badge href="publications/learn-dvi.pdf" >}}PDF{{< /badge >}}
  {{< badge href="https://hal.science/hal-05290903v1" >}}HAL{{< /badge >}}
  {{< badge href="https://dx.doi.org/10.1007/s10444-026-10336-1" >}}doi{{< /badge >}}
  {{< badge href="https://github.com/tremelow/symplearn/releases/tag/preprint" >}}code{{< /badge >}}

- Brigitte Bidégaray-Fesquet, Clément Jourdana, LT, "Multi-frequency averaging and uniform accuracy towards numerical approximations for a Bloch model" (2023). In *Communications in Mathematical Sciences*.
  {{< badge href="publications/bloch-ua.pdf" >}}PDF{{< /badge >}}
  {{< badge href="https://hal.univ-grenoble-alpes.fr/hal-04171818" >}}HAL{{< /badge >}}
  {{< badge href="https://dx.doi.org/10.4310/CMS.250208224637" >}}doi{{< /badge >}}

- Philippe Chartier, Mohammed Lemou, LT, "A uniformly accurate numerical method for a class of dissipative systems" (2021). In *Mathematics of Computation*.
  {{< badge href="publications/relax-mima.pdf" >}}PDF{{< /badge >}}
  {{< badge href="https://doi.org/https://doi.org/10.1090/mcom/3688" >}}DOI{{< /badge >}}

{{< lead >}}
Recent talks
{{< /lead >}}

- WCCM-ECCOMAS 2026, MS187: Structure-Preserving Scientific Machine Learning and Neural Networks. "Neural Non-Canonical Hamiltonian Dynamics for Long-Time Simulations"
  {{< badge href="slides/2026-07-20-WCCM-ECCOMAS-LTremant.pdf">}}slides{{< /badge >}}

- CANUM 2026. "Uniform accuracy from geometric high-order
averaging"
  {{< badge href="slides/2026-01-09-MAP5-LTremant.pdf">}}slides{{< /badge >}}

- Groupe de travail Analyse, Modélisation, Simulation, MAP5 Paris-Cité. "Uniform accuracy from geometric high-order
averaging"
  {{< badge href="slides/2026-06-02-CANUM-LTremant.pdf">}}slides{{< /badge >}}

- NODYCON 2025. "Learning non-canonical Hamiltonian dynamics"
  {{< badge href="slides/2025-06-22-NODYCON-LTremant.pdf">}}slides{{< /badge >}}

- Numkin 2024. "Learning non-canonical Hamiltonian dynamics for the guiding center problem"
  {{< badge href="slides/2024-11-06-Numkin-LTremant.pdf">}}slides{{< /badge >}}

{{< lead >}}
Thesis
{{< /lead >}}

- "Méthodes d’analyse asymptotique et d’approximation numérique" (2021). PhD thesis.
  {{< badge href="phd/phd-thesis_LTremant.pdf" >}}PDF{{< /badge >}}
  {{< badge href="phd/phd-slides_LTremant.pdf" >}}slides{{< /badge >}}
